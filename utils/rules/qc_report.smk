...
rule qc_report2:
    ...
    output:
        b=config["output_dir"]+"/QC_html_report/"+"qc_report_2.html",
    shell:
        """
        Rscript -e 'rmarkdown::render(input="{params.path}/utils/scripts/dada2/qc_report_3.Rmd",output_file="{params.path}/{output.a}", params= list(seqtab="{params.seqtab}", taxonomy="{params.taxonomy}", krona="{params.krona}", path="{params.path}", length_dist="{params.length_distribution}", out_dir="{params.outdir}"))'
        """
...
rule qc_report3:
    ...
    output:
    	c=config["output_dir"]+"/QC_html_report/"+"qc_report_3.html",
    shell:
        """
        Rscript -e 'rmarkdown::render(input="{params.path}/utils/scripts/dada2/qc_report_2.Rmd",output_file="{params.path}/{output.a}", params= list(seqtab="{params.seqtab}", taxonomy="{params.taxonomy}", pos="{params.pos}", ref="{params.ref}", source="{params.source}"))'
        """
